...
numGenotype = length(genotypesToAlleles)

genotypeFactor.card = [numGenotype,numGenotype,numGenotype]
genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
for i = 1:numGenotype
	for j = 1:numGenotype
		combination = [genotypesToAlleles(i,1),genotypesToAlleles(j,1);genotypesToAlleles(i,1),genotypesToAlleles(j,2);genotypesToAlleles(i,2),genotypesToAlleles(j,1);genotypesToAlleles(i,2),genotypesToAlleles(j,2);]
		for k = 1:4
			alleleIDOne = combination(k,1)
			alleleIDTwo = combination(k,2)
			genotypeID = allelesToGenotypes(alleleIDOne,alleleIDTwo)
			index = AssignmentToIndex([genotypeID,i,j],genotypeFactor.card)
			genotypeFactor.val(index) += 0.25
		end
	end
end
